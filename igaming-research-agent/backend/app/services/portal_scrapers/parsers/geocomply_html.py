...
class GeocomplyHtmlParser(PortalListingParser):
    """Parser for GeoComply press coverage pages."""

    def matches(self, source_url: str, company_name: str) -> bool:
        token_url = (source_url or "").lower()
        token_name = (company_name or "").lower()
        return "geocomply.com/awards-and-press" in token_url or "geocomply" in token_name
    ...
    def parse_listing(
        ...
        links = soup.select("a[href*='/news/'][href]")

        if not links:
            result.empty_reason = "no_geocomply_release_links"
            return result
        ...
        for index, link in enumerate(links):
            ...
            if published is not None and cutoff is not None and published < cutoff:
                if index == 0:
                    result.empty_reason = "listing_first_geocomply_item_outside_time_window"
                    return result
                break
    ...
    def _parse_date(value: str) -> datetime.datetime | None:
        match = re.search(r"\b([A-Za-z]{3}\s+\d{1,2},\s+\d{4})\b", value or "")
        if not match:
            return None
        try:
