...
class CaesarsHtmlParser(PortalListingParser):
    """Dedicated parser for Caesars investor press releases."""
    ...
    def matches(self, source_url: str, company_name: str) -> bool:
        ...
        return "investor.caesars.com/press-releases" in token_url or "caesars" in token_name
    ...
    def parse_listing(
        ...
        if not links:
            result.empty_reason = "no_caesars_release_links"
            return result
        ...
        for index, link in enumerate(links):
            ...
            if published is not None and cutoff is not None and published < cutoff:
                if index == 0:
                    result.empty_reason = "listing_first_caesars_item_outside_time_window"
                    return result
                break
        ...
        if not result.candidate_urls and result.empty_reason is None:
            result.empty_reason = "no_caesars_in_window_links"
