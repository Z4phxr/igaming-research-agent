...
class Ic360HtmlParser(PortalListingParser):
    """Parser for IC360 media listings."""

    def matches(self, source_url: str, company_name: str) -> bool:
        token_url = (source_url or "").lower()
        token_name = (company_name or "").lower()
        return "ic360.io/media" in token_url or "ic360" in token_name
    ...
    def parse_listing(
        ...
        links = soup.select("a[href*='ic360.io/page/'][href], a[href^='/page/'][href], a[href^='./page/'][href]")

        if not links:
            if "latest media" in html.lower() and "./page/" in html.lower():
                result.empty_reason = "dynamic_listing_no_static_links"
                return result
            result.empty_reason = "no_ic360_release_links"
            return result
        ...
        for index, link in enumerate(links):
            ...
            if published is not None and cutoff is not None and published < cutoff:
                if index == 0:
                    result.empty_reason = "listing_first_ic360_item_outside_time_window"
                    return result
                break
    ...
    def _parse_date(value: str) -> datetime.datetime | None:
        match = re.search(r"\b([A-Za-z]{3,9}\s+\d{1,2},\s+\d{4})\b", value or "")
        if not match:
            return None
