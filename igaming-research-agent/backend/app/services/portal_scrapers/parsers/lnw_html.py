...
class LnwHtmlParser(PortalListingParser):
    """Dedicated parser for Light & Wonder newsroom."""

    def matches(self, source_url: str, company_name: str) -> bool:
        token_url = (source_url or "").lower()
        token_name = (company_name or "").lower()
        return "explore.lnw.com/newsroom" in token_url or "light & wonder" in token_name or "lnw" in token_name
    ...
    def parse_listing(
        ...
        links = soup.select("a[href*='/newsroom/']:not([href$='/newsroom/']):not([href*='/newsroom/boilerplate']):not([href*='/newsroom/awards'])")

        if not links:
            result.empty_reason = "no_lnw_release_links"
            return result
        ...
        for index, link in enumerate(links):
            ...
            if published is not None and cutoff is not None and published < cutoff:
                if index == 0:
                    result.empty_reason = "listing_first_lnw_item_outside_time_window"
                    return result
                break
        ...
        if not result.candidate_urls and result.empty_reason is None:
            result.empty_reason = "no_lnw_in_window_links"
    ...
    @staticmethod
    def _parse_mmddyyyy(value: str) -> datetime.datetime | None:
