...
class CatenaMediaHtmlParser(PortalListingParser):
    """Parser for Catena Media press releases."""

    def matches(self, source_url: str, company_name: str) -> bool:
        token_url = (source_url or "").lower()
        token_name = (company_name or "").lower()
        return "catenamedia.com/investors/press-releases" in token_url or "catena media" in token_name
    ...
    def parse_listing(
        ...
        links = soup.select("a[href*='/release/'][href]")

        if not links:
            if self._is_bot_blocked(html):
                result.empty_reason = "bot_protection_blocked"
                return result
            result.empty_reason = "no_catenamedia_release_links"
            return result
        ...
        for index, link in enumerate(links):
            ...
            if cutoff is not None and published < cutoff:
                if index == 0:
                    result.empty_reason = "listing_first_catenamedia_item_outside_time_window"
                    return result
                break
    ...
    def _parse_date(value: str) -> datetime.datetime | None:
        match = re.search(r"\b([A-Z]+\s+\d{1,2}(?:ST|ND|RD|TH),\s+\d{4})\b", value or "")
        if not match:
            return None
        cleaned = re.sub(r"(\d)(ST|ND|RD|TH)", r"\1", match.group(1), flags=re.IGNORECASE)
        try:
            return datetime.datetime.strptime(cleaned.title(), "%B %d, %Y")
        except ValueError:
            return None
