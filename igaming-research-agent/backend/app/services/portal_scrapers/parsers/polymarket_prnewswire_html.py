...
class PolymarketPrnewswireHtmlParser(PortalListingParser):
    """Parser for PR Newswire Polymarket feed pages."""

    def matches(self, source_url: str, company_name: str) -> bool:
        token_url = (source_url or "").lower()
        token_name = (company_name or "").lower()
        return "prnewswire.com/news/polymarket" in token_url or "polymarket" in token_name
    ...
    def parse_listing(
        ...
        if "doubleclick.net/activityi" in lower:
            result.empty_reason = "tracking_redirect_blocked"
            return result

        soup = BeautifulSoup(html, "html.parser")
        links = soup.select("a[href*='/news-releases/'][href]")
        if not links:
            result.empty_reason = "no_polymarket_release_links"
            return result
        ...
        for index, link in enumerate(links):
            ...
            if len(title) < 8:
                continue
            if "polymarket" not in title.lower() and "polymarket" not in absolute_url.lower():
                continue
            ...
            if published is not None and cutoff is not None and published < cutoff:
                if index == 0:
                    result.empty_reason = "listing_first_polymarket_item_outside_time_window"
                    return result
                break
