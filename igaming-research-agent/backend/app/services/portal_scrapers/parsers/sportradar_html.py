...
class SportradarHtmlParser(PortalListingParser):
    """Parser for Sportradar content-hub news entries."""

    def matches(self, source_url: str, company_name: str) -> bool:
        token_url = (source_url or "").lower()
        token_name = (company_name or "").lower()
        return "sportradar.com/content-hub" in token_url or "sportradar" in token_name
    ...
    def parse_listing(
        ...
        links = soup.select("a[href*='/content-hub/news/'][href]")

        if not links:
            if self._is_bot_blocked(html):
                result.empty_reason = "bot_protection_blocked"
                return result
            result.empty_reason = "no_sportradar_release_links"
            return result
        ...
        for index, link in enumerate(links):
            ...
            if published is not None and cutoff is not None and published < cutoff:
                if index == 0:
                    result.empty_reason = "listing_first_sportradar_item_outside_time_window"
                    return result
                break
    ...
    def _is_bot_blocked(html: str) -> bool:
        lower = (html or "").lower()
        return "cloudflare" in lower or "captcha" in lower or "access denied" in lower
