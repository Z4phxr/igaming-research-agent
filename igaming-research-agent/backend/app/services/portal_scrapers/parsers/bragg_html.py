...
class BraggHtmlParser(PortalListingParser):
    """Parser for Bragg Group news listing pages."""

    def matches(self, source_url: str, company_name: str) -> bool:
        token_url = (source_url or "").lower()
        token_name = (company_name or "").lower()
        return "bragg.group/news" in token_url or "bragg" in token_name
    ...
    def parse_listing(
        ...
        links = soup.select("a[href*='bragg.group/'][href]")

        if not links:
            result.empty_reason = "no_bragg_release_links"
            return result
        ...
        for index, link in enumerate(links):
            ...
            if absolute_url in seen:
                continue
            if not path or path == "news" or path.startswith("news/page"):
                continue
            ...
            # Real news rows include a nearby publish date; generic site links do not.
            if published is None:
                continue
            if published is not None and cutoff is not None and published < cutoff:
                if index == 0:
                    result.empty_reason = "listing_first_bragg_item_outside_time_window"
                    return result
                break
